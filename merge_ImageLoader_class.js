...
class AssetManager {
    constructor(itemDefinitions, flyersDefinitions) {
        this.imageLoader = new ImageLoader();
        this.imagePaths = this.collectAllImagePaths(itemDefinitions, flyersDefinitions);
    }

    collectAllImagePaths(items, flyers) {
        ...
        for (const category of Object.values(flyers)) {
            if (category.set) {
                for (const level of Object.values(category.set)) {
                    if (level.pic && typeof level.pic === 'string' && level.pic.endsWith('.png')) {
                        paths.add(level.pic);
